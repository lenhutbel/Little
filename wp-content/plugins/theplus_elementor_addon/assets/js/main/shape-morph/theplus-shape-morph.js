(function ($) {
	...
	var WidgetMorphingLayoutHandler = function($scope, $) {
		...
		var wid_sec=$scope.closest('section.elementor-element,.elementor-element.e-container');		
		if(wid_sec.length){
			//fixed Morph
			...
			//sections Morph
			if($scope.closest('section.elementor-element,.elementor-element.e-container').find('> [data-morph-id="'+morph_id+'"]').length > 0){
				var remove_morph=$scope.closest('section.elementor-element,.elementor-element.e-container').find("> .plus-morphing-svg-wrapper");
				remove_morph.remove();
			}
			//column Morph
		...
		if(morph_row=='yes' && morph_row!=undefined){
			morphing_wrap.closest('section.elementor-element,.elementor-element.e-container').prepend(morphing_wrap);
		}
