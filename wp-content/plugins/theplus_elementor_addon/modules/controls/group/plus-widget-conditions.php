...
class Theplus_Widgets_Rules extends Elementor\Widget_Base {
		...
		if(class_exists('woocommerce')) {
			$this->rules_options[] = [
				...
				'options' 				=> [
					...
					//'woo_cur_cat_page_cart' 	=> esc_html__( 'Woo : Category In Cart', 'theplus' ),
	...
	public function add_rules_controls( $element, $args ) {
		...
		$element->start_controls_section(
			...
			[
				'label' => esc_html__( 'Plus Extras : Display Rules', 'theplus' ),
		...
		if(class_exists('woocommerce')) {
			// $repeater->add_control(
			// 	'tp_rule_woo_cur_cat_page_cart_value',
			// 	[
			// 		'type' 			=> Controls_Manager::SELECT2,
			// 		'default' 		=> array(),
			// 		'label_block' 	=> true,
			...
			// 		'condition' 	=> [
			// 			'tp_rule_key' => 'woo_cur_cat_page_cart',
			// 		],
			// 	]
			// );	
			$repeater->add_control(
	...
	protected function set_rules( $id, $rules = [] ) {
		...
		foreach ( $rules as $index => $rule ) {
			...
			if ( method_exists( $this, 'plus_check_' . $key ) ) {
				$check = call_user_func( [ $this, 'plus_check_' . $key ], $value, $check_is_not,$key_name ,$mode, $shortcodeoutput);
	 ...
	 * @param string $check_is_not  Comparison value.
	 */
	// protected static function plus_check_woo_cur_cat_page_cart( $value, $check_is_not, $key ) {	
	// 	$display = false;	
	// 	$cart = WC()->cart;			
	// 	if($cart->is_empty()){
	// 		return false;
	...
	// 		}			
	// 		if($product->get_category_ids() == $value){
	// 			$display = true;
	// 		}
	// 	}
