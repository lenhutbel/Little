    ...
    protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				//         [
				//         'terms' => [
				//         	    ['name' => 'mobile_menu_content', 'operator' => '!=', 'value' => 'template-menu'], 
				//             ]
				//         ],
				//         [
				//         'terms' => [
				//                 ['name' => 'mobile_menu_type', 'operator' => '==', 'value' => 'off-canvas'],
				//             ]
				//         ],
				//     ]
				// ],
	...
	protected function render() {
		...
		<div class="plus-navigation-wrap <?php echo esc_attr($nav_alignment); ?> <?php echo esc_attr($uid); ?>">
			<div class="plus-navigation-inner <?php echo esc_attr($menu_hover_click); ?> <?php echo esc_attr($main_menu_indicator_style); ?> <?php echo esc_attr($sub_menu_indicator_style); ?> <?php echo esc_attr($menu_effect); ?>" <?php echo $menu_attr; ?>>
				...
				<?php if($settings["show_mobile_menu"]=='yes'){ ?>
					...
					?>
					<div id="plus-mobile-nav-toggle-<?php echo esc_attr($uid); ?>" class="plus-mobile-menu plus-menu-off-canvas collapse navbar-collapse navbar-ex1-collapse plus-mobile-menu-content <?php echo $mobile_nav_custom_class .esc_attr($swiper_class); ?>">
						<?php
							if(!empty($settings["mobile_menu_type"]) && $settings["mobile_menu_type"] == 'off-canvas'){
							...
							if( $settings["mobile_menu_content"]=='normal-menu' && !empty($settings["mobile_navbar"]) && !empty($settings["mobile_menu_type"]) && $settings["mobile_menu_type"]=='swiper' ){
								...
								if(defined("JUPITERX_VERSION")){
									...
									echo $this->tp_mega_menu($settings,1);
						...
						<?php if((!empty($settings["mobile_menu_type"]) && $settings["mobile_menu_type"] == 'off-canvas') || $settings["mobile_menu_content"]=='template-menu' && !empty($settings['mobile_navbar_template'])){
		...
		if($settings["show_mobile_menu"]=='yes' && !empty($settings['open_mobile_menu']['size'])){
			...
			if((!empty($settings['enable_sticky_menu']) && $settings['enable_sticky_menu']=='yes') && (!empty($settings['tp_mob_scroll_overflow']) && $settings['tp_mob_scroll_overflow']=='yes')){
				$css_rule .='@media (max-width:'.esc_attr($open_mobile_menu).'){.elementor-section.plus-nav-sticky-sec.plus-fixed-sticky,.elementor-element.e-container.plus-nav-sticky-sec.plus-fixed-sticky{overflow:scroll;height: 100%}}';
			}
		...
		if((!empty($settings['enable_sticky_menu']) && $settings['enable_sticky_menu']=='yes' ) && (!empty($smain_height_size)) && (!empty($settings['enable_sticky_osup_menu']) && $settings['enable_sticky_osup_menu'] !='yes')){
			...
			.elementor-section.plus-nav-sticky-sec.plus-fixed-sticky,.elementor-element.e-container.plus-nav-sticky-sec.plus-fixed-sticky{top: -'.$smain_height_size.'px !important;-webkit-transform: translate3d(0,'.$smain_height_size.'px,0);transform: translateY('.$smain_height_size.'px) !important;transition: all .3s linear !important;}';
			
			$css_rule .='.admin-bar .elementor-section.plus-nav-sticky-sec.plus-fixed-sticky,.elementor-element.e-container.plus-nav-sticky-sec.plus-fixed-sticky{top: calc(-'.$smain_height_size.'px + 32px) !important;-webkit-transform: translate3d(0,'.$smain_height_size.'px,0);transform: translateY('.$smain_height_size.'px) !important;transition: all .3s linear !important;}';			
		}
	...
	protected function tp_mega_menu($settings,$sett='') {
		...
		if(!empty($settings['ItemMenu'])){
			...
			foreach($settings['ItemMenu'] as $index => $item){
				...
				if($settings['main_menu_indicator_style'] == 'style-1' && $depth == '0' && $Nextdepth!='' && $Nextdepth > 0 && $Nextdepth != $depth){
					$indiIcon .= '<span><i class="'.($settings['navbar_menu_type'] == 'vertical-side' ? 'fa fa-angle-right' : 'fa fa-angle-down' ).'"></i></span>';
				}

				if($depth >= 1  && $Nextdepth > 1 && $Nextdepth != $depth && $Nextdepth > $depth){
					$subindiIcon .= '<span>';
						if($settings['sub_menu_indicator_style'] == 'style-1'){
							$subindiIcon .= '<i class="fa fa-angle-right"></i>';
						}
					$subindiIcon .= '</span>';
				}
				if(($depth != '1') || !empty($item['SmenuType']) && $item['SmenuType'] != 'mega-menu' && $item['SmenuType'] == 'link') {
					$name = '<a href="'.esc_attr($itemUrl).'" '.$itemTarget.$itemNofollow.' title="'.esc_attr($menuName).'" data-text="'.esc_attr($menuName).'" >'.$preicon.'<span class="plus-title-wrap">'.esc_html($menuName).''.$txtLabel.''.$indiIcon.''.$subindiIcon.''.$navdesc.'</span></a>';
				}
